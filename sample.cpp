...
InitGraphics( )
{
	...
	if (texture == NULL) { std::cout << "Texture cannot be loaded" << std::endl; }
	glBindTexture(GL_TEXTURE_3D, Noise3);
	...
	//bool valid = Pattern.Create( "terrainnoise.vert", "terrainnoise.frag" ); 
	//bool valid = Pattern.Create("hedgehog.vert", "hedgehog.frag", "hedgehog.geom");
	bool valid = Pattern.Create("sphereadapt.vert", "sphereadapt.frag", "sphereadapt.geom", "sphereadapt.tcs", "sphereadapt.tes");
	if( !valid )
	...
	Pattern.SetUniformVariable( "uKd", 0.9f );
	Pattern.SetUniformVariable( "uKs", 0.4f );
	Pattern.SetUniformVariable("uLevel", 1);
	Pattern.SetUniformVariable( "uColor", 1.0f, 0.5f, 0.15f);
	Pattern.SetUniformVariable( "uSpecularColor", 1.f, 1.f, 1.f );
	Pattern.SetUniformVariable( "uShininess", 12.f );

	Pattern.SetUniformVariable("uShrink", 0.9f);
	Pattern.SetUniformVariable("uDetail", 30.0f);
	Pattern.SetUniformVariable("uNoiseAmp", 0.5f);
	Pattern.SetUniformVariable("uNoiseFreq", 0.4f);
	Pattern.SetUniformVariable("uBiasx", 0.1f);
	Pattern.SetUniformVariable("uBiasy", 1.0f);
	Pattern.SetUniformVariable("uBiasz", 0.1f);

	Pattern.SetUniformVariable("uLevel1", 0.3f);
	Pattern.SetUniformVariable("uLevel2", 0.3f);
	Pattern.SetUniformVariable("uTol", 0.3f);

	Pattern.SetUniformVariable("uLightX", 0.0f);
	...
	unsigned char* textureArray0 = BmpToTexture("lroc_color_poles_1k.bmp", &width0, &height0);
	unsigned char* textureArray1 = BmpToTexture("ldem_3_8bit.bmp", &width1, &height1);

	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1, &Tex0); // assign binding “handles” to texture objects
	glGenTextures(1, &Tex1);

	glBindTexture(GL_TEXTURE_2D, Tex0); // make Tex0 the current texture and store its parametersglTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
	...
	glBindTexture(GL_TEXTURE_2D, Tex1); // make Tex1 the current texture and store its parametersglTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
InitLists( )
{
	...
	TESS = glGenLists(1);
	glNewList(TESS, GL_COMPILE);
		glBegin(GL_PATCHES);
			glVertex4f(0, 0, 0, 0.6f);
			//glVertex4f(0, 1, 0, 0.3f);
			//glVertex4f(0, 0, 1, 0.4f);
		glEnd();
	glEndList();
